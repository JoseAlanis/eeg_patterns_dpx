...
# mid range effects
ay_ax_mid_positive <- report.t.values(cue_peaks, N, time = 'midrange', polarity = 'positive')
ay_ax_mid_negative <- report.t.values(cue_peaks, N, time = 'midrange', polarity = 'negative')

# late/later effects
ay_ax_late_positive <- report.t.values(cue_peaks, N, time = 'late', polarity = 'positive')
ay_ax_late_negative <- report.t.values(cue_peaks, N, time = 'late', polarity = 'negative')
```

As shown in Figure 5, we found significant differences in the amplitude responses
elicited by "X" and "Y" probes when they were presented after an "A" cue
(i.e., between "AX" and "AY" cue-probe pairs). The earliest differences emerged
at approximately 180 to 250 ms post-porbe. In this time window, "Y" probes
showed a stronger negative polarisation of the evoked amplitude response at
...
`r ay_ax_early_negative$d`, `r ay_ax_early_negative$dci`; see Figures 6a and 6d).
...
(ref:fig4-caption) Results of the mass univariate analysis of the probe-evoked
amplitude response for the contrast "AY" minus "AX".
\newline Depicted are the results of a paired-samples t-test contrasting "Y"
...
p2 <- ggdraw() +
  draw_image("/media/administrator/Crucial X8/code/eeg_patterns_dpx/data/bids/derivatives/limo/ttest_probes_AY_AX_tvals_fmax.png",
             scale = 1, hjust=0)
